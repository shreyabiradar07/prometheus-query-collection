...
get_queries() {
    ...
    case "$query_set" in
        "individual")
            return_array=("${individual_queries[@]}")
            ;;
        "grouped")
        ...
        "grouped_5days")
            ...
            ;;
        *)
            echo "Invalid query set. Available sets are: individual, grouped, grouped_5days."
...
capture_prometheus_resource_metrics() {
    echo -e "\n===== Prometheus Pod Resource Metrics (CPU & Memory) =====" >> "$OUTPUT_FILE"
    # Get resource usage for Prometheus pods in the given namespace
    kubectl top pod -n "$PROMETHEUS_NAMESPACE" | grep "prometheus" >> "$OUTPUT_FILE"
    echo -e "=========================================================\n" >> "$OUTPUT_FILE"
}
...
DEFAULT_QUERY_SET="individual"
DEFAULT_START_TIME='1730874056'
...
OUTPUT_FILE="prometheus_${QUERY_SET}_query_stats.txt"
RESPONSE_LOG_FILE="${QUERY_SET}_query_response.log"
...
if [[ "$QUERY_SET" == "grouped_5days" ]]; then
